...
public class MyVehicleCommAdapter
    extends
      BasicVehicleCommAdapter
    implements
      MqttCallback {

  private static final Logger LOG = LoggerFactory.getLogger(MyVehicleCommAdapter.class);
  ...
  public MyVehicleCommAdapter(
    ...
    vehicleProcessModel.setState(Vehicle.State.IDLE);
  }
  ...
  public void sendCommand(MovementCommand command) {
    try {
      ...
      // {"command":"move","destination":"Point-0002","routeIndex":0}
      ...
      String topic = "vehicle/control";
      mqttClient.publish(topic, message);
      LOG.info("Sent command to topic {}: {}", topic, commandJson);

      // Auto-complete the command for simulation purposes
      // In a real MQTT setup, this would be triggered by a response from the vehicle
      getExecutor().execute(() -> {
        try {
          // Simulate some processing time
          Thread.sleep(500);
          LOG.info("Auto-completing command: {}", command);
          ...
          if (!getSentCommands().isEmpty()) {
            ...
            getProcessModel().setPosition(command.getStep().getDestinationPoint().getName());
          }
        }
        catch (InterruptedException e) {
          LOG.warn("Command completion thread interrupted", e);
        }
      });
    }
    catch (MqttException e) {
  ...
  public void connectionLost(Throwable cause) {
    LOG.error("Connection to MQTT broker lost. Will attempt to reconnect automatically...", cause);
  ...
  public void processMessage(
      org.opentcs.drivers.vehicle.VehicleCommAdapterMessage message
  ) {
    // Handle INIT_POSITION message to set vehicle position
    if ("tcs:virtualVehicle:initPosition".equals(message.getType())) {
      String position = message.getParameters().get("position");
      if (position != null) {
        LOG.info("Setting vehicle position to: {}", position);
